...
function character_vert_init()
	...
	if (_set_properties)
		...
		//Hurtbox
		hurtbox_sprite = spr_vert_hurtbox;
		...
		//Items
		item_hold_x_default = 12;
		item_hold_y_default = 0;
		
		//Custom Scripts
		draw_script = -1;
		callback_add(callback_passive, command_inputs_passive, CALLBACK_TYPE.permanent);
		}
	...
	if (_set_attacks)
		{
		my_attacks[$ "Jab"			] = basic_jab_rapid;
		...
		my_attacks[$ "Fsmash"		] = vert_fsmash;
		my_attacks[$ "Usmash"		] = sheik_usmash;
		my_attacks[$ "Dsmash"		] = pichu_dsmash;
				 
		my_attacks[$ "Nair"			] = dedede_nair;
		my_attacks[$ "Fair"			] = mewtwo_fair;
		my_attacks[$ "Bair"			] = mario_bair;
		my_attacks[$ "Uair"			] = mario_uair;
		my_attacks[$ "Dair"			] = falco_dair;
				 
		my_attacks[$ "Nspec"		] = vert_nspec_homing_attack;
		my_attacks[$ "Fspec"		] = falcon_fspec;
		my_attacks[$ "Uspec"		] = fox_uspec;
		my_attacks[$ "Dspec"		] = snake_dspec_c4;
		...
		my_attacks[$ "Bthrow"		] = basic_bthrow_drag;
		my_attacks[$ "Uthrow"		] = basic_uthrow_jump;
		...
		my_attacks[$ "Taunt"		] = vert_taunt;
		my_attacks[$ "Final_Smash"	] = vert_final_smash;
		}
	...
	if (_set_sprites)
		...
		my_sprites[$ "Entrance"			] = anim_define(spr_vert_entrance, anim_define_ext(spr_vert_idle, 0, 0.18));
		my_sprites[$ "Idle"				] = anim_define_ext(spr_vert_idle, 0, 0.18);
		...
		my_sprites[$ "Shield_Release"	] = spr_vert_idle;
