function departure_forward()
	{
	//Forward version of departure. Spawns at waist height and shoots straight forward
	...
	if (run)
		...
		switch (_phase)
			...
			case PHASE.start:
				...
				var _exists = false;
				with (obj_departure_forward_spike)
					{
					if (owner == other.id)
						{
						_exists = true;
			...
			case 0:
				...
				if (attack_frame == 0)
					...
					spike = entity_create(x + 24 * facing, y - 128, obj_departure_forward_spike, "Player_Front");
					spike.custom_entity_struct.facing = facing
					}
				break;
				}
			//Finish
