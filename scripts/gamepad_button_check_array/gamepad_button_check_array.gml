...
/*
If a single button is given, it returns true if that button is being held down.
If an array of buttons is given, it returns true if any of those buttons are being held down.
*/
function gamepad_button_check_array()
	{
	...
	if (!is_array(_button))
		...
		for (var i = 0; i < array_length(_button); i++)
			{
			if (gamepad_button_check(_device, _button[@ i]))
				{
				return true;
				}
			}
