...
/*
Standard script for when an <obj_hitbox_projectile> hits a player's hurtbox.
...
function hurtbox_projectile_hit_player()
	...
	switch (_hurtbox.inv_type)
		...
		case INV.heavyarmor:
			...
			stored_hitstun = calculate_hitstun(_hitbox.base_knockback, _hitbox.damage, weight_multiplier, damage, _hitbox.knockback_scaling, hitstun_multiplier, _hitbox.custom_hitstun);
			...
			//Normal
				...
				if (_hitbox.can_lock && state == PLAYER_STATE.knockdown)
					...
					_hitbox.self_hitlag_frame = _hitbox.base_hitlag;
					}
				else
					{
					//Change state
		...
		case INV.shielding:
			//Register hit
			hitbox_register_hit(_hitbox, true, false);
			//Deplete other's shield
			shield_hp -= _hitbox.custom_shield_damage != -1 ? _hitbox.custom_shield_damage : _hitbox.damage;
			var _stun = calculate_shieldstun(_hitbox.damage, _hitbox.shieldstun_scaling);
			shieldstun = max(shieldstun, _stun);
			_hitbox.self_hitlag_frame = _stun;
			//Pushback
			...
			//Effects
			hit_vfx_style_create(HIT_VFX.shield_projectile, 0, _hitbox, 0);
			hit_sfx_play(shield_hit_sound);
			break;
		case INV.powershielding:
			//Don't register a hit
			//Change owner
			hitbox_owner_change(_hitbox, id);
			//Reflect
			_hitbox.hsp = -_hitbox.hsp * powershield_reflect_multiplier;
			_hitbox.vsp = -_hitbox.vsp * powershield_reflect_multiplier;
		...
		case INV.parry_press:
			...
			hitbox_owner_change(_hitbox, id);
			//Trigger the parry
			parry_press_trigger(_hitbox, false, id);
			//Reflect
			if (_hitbox.can_be_parried)
				{
				_hitbox.hsp = -_hitbox.hsp * parry_press_reflect_multiplier;
				_hitbox.vsp = -_hitbox.vsp * parry_press_reflect_multiplier;
				_hitbox.facing = -_hitbox.facing;
				}
			break;
		case INV.parry_shield:
			...
			parry_shield_trigger(_hitbox, false, id);
			break;
		case INV.counter:
			//Register hit
			hitbox_register_hit(_hitbox, true, false);
