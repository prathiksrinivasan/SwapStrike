...
ggmr_custom_init
	...
	function()
		...
		switch (_struct.packet_type)
			{
			case "ready_up_data":
				assert(engine().online_is_leader, "[obj_spectator_wait_ui: Room Start] Cannot receive a 'ready_up_data' packet when not the leader!");
				...
				break;
			case "ready_up":
				assert(!engine().online_is_leader, "[obj_spectator_wait_ui: Room Start] Received a 'ready_up' packet despite being the leader!");
				//Set the player's ready property
				var _player_number = _struct.player_number;
				var _player = engine().css_player_data[@ _player_number];
				_player[@ CSS_PLAYER.custom].ready = true;
				...
				break;
			case "start_data":
				assert(!engine().online_is_leader, "[obj_spectator_wait_ui: Room Start] Received a 'start_data' packet despite being the leader!");
				
				//Player data
				var _css_data = _struct.css_data;
				var _num = array_length(_css_data);
				assert(_num == css_players_count(), "[obj_spectator_wait_ui: Room Start] Number of players in the received start data (", _num, ") != the local CSS data (", css_players_count(), ")");
			...
			case "disconnect":
				...
				break;
			default: log("[obj_spectator_wait_ui: Room Start] Received a packet with an unrecognized packet_type: ", _struct.packet_type); break;
			}
		}
	);
